...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //    OR 
  // 2   2) use the Quaternion<float> class, which has a handy FromEuler123_RPY function for creating a quaternion from Euler Roll/PitchYaw
  ...
  */
  ...
  Quaternion<float> quarternion = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  quarternion.IntegrateBodyRate(gyro,dtIMU);

  float predictedPitch = quarternion.Pitch();
  float predictedRoll = quarternion.Roll();
  ekfState(6) = quarternion.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) = predictedState(3) + velocity_u.x;
  predictedState(4) = predictedState(4) + velocity_u.y;
  predictedState(5) = predictedState(5) - gravity*dt + velocity_u.z;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime << -cosf(roll)*sinf(yaw),-sinf(pitch)*sinf(roll)*sinf(yaw) - cosf(pitch)*cosf(yaw), -cosf(pitch)*sinf(roll)*sinf(yaw) + sin(pitch)*cosf(yaw),
			   cosf(roll)*cosf(yaw), sinf(pitch)*sinf(roll)*cos(yaw) - cosf(pitch)*sinf(yaw), cosf(pitch)*sinf(roll)*cosf(yaw) + sinf(pitch)*sinf(yaw),
			   0,0,0;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  VectorXf u = VectorXf(3);
  u << accel.x, accel.y, accel.z;
  VectorXf mul_term = (RbgPrime * u) * dt;
  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = mul_term[0];
  gPrime(4, 6) = mul_term[1];
  gPrime(5, 6) = mul_term[2];
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  z(0) = yaw_error / 2.f;
  zFromX(0) = -yaw_error / 2.f;
  hPrime << 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 1.f;
